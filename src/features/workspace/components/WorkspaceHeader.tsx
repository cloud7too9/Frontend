...
export function WorkspaceHeader() {
  ...
  const speichereWorkspace = useWorkspaceStore((s) => s.speichereWorkspace);
  const speicherLaeuft = useWorkspaceStore((s) => s.speicherLaeuft);
  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-surface-muted px-4 py-3">
      <div className="flex items-baseline gap-3">
        ...
        <span className="text-sm text-text-muted">Workspace · {workspaceName}</span>
      </div>
        ...
        {editMode && (
          <>
            ...
            <Button
              variant="ghost"
              onClick={() => {
                void speichereWorkspace();
              }}
              disabled={speicherLaeuft}
            >
              {speicherLaeuft ? "Speichert…" : "Speichern"}
            </Button>
          </>
        )}
        <Button
